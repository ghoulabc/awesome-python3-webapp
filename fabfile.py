...
env.host_string = '192.168.1.18' # 改成你的服务器ip
db_user = 'www-data'
...
def deploy():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo('chown ghoul:ghoul www') # user改为你的linux服务器上的用户名
        sudo('chown -R ghoul:ghoul %s' % newdir) # 同上
...
def backup():
    ...
    with cd('/tmp'):
        sudo('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick awesome > %s' % (db_user, db_password, f))
        sudo('tar -czvf %s.tar.gz %s' % (f, f))
        get('%s.tar.gz' % f, '%s/backup/' % _current_path())
        sudo('rm -f %s' % f)
        sudo('rm -f %s.tar.gz' % f)
def restore2local():
    '''
    Restore db to local
    '''
