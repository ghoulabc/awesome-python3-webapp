...
# 定义request常用方法的传参装饰器，放在函数上用来与app携带的method匹配，并将path参数传入函数
def get(path):
    '''
...
# 用来判断request携带的参数以判断是什么类型的request以匹配适合的响应函数
...
class RequestHandler(object):
    ...
    async def __call__(self, request):
        kw = None
        # 通过request携带的的参数类型和方法类型判断参数是否合法
        # 不合法返回相应的响应错误，合法则将参数传入响应函数
